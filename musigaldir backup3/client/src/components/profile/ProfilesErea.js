...
const ProfilesErea = () => {
  ...
  const onFollow = async() => {
    if(!isFollow){
      setIsFollow(true)
      try {
      const data = await axios.put(`http://localhost:3001/users/follow/${id}`,{},
      {
    ...
    setIsFollow(false)
    try {
      const data = await axios.put(`http://localhost:3001/users/unfollow/${id}`,{},
      {
        headers: {"x-api-key": localStorage.getItem("token")}
      })
    } catch (error) {
          ...
          <div className="" style={{ backgroundColor: "#EEEDEF" }}>
            ...
            >
              <div className="d-flex ms-5 ">
                ...
                {loggedInUser?._id == user._id ? <div></div> : 
                <button onClick={() => onFollow()}>{isFollow ? (<div>Unfollow</div>) : (<div>follow</div>)}</button>}
              </div>
            </div>
